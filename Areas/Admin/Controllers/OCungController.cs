...
namespace ThanhThoaiRestaurant.Areas.Admin.Controllers
{
    ...
    public class AdminOCungController : Controller
    {
        private readonly QuanLyNhaHangContext _context;
        private readonly IWebHostEnvironment _webHostEnvironment;
        public AdminOCungController(QuanLyNhaHangContext context, IWebHostEnvironment webHostEnvironment)
        {
            _context = context;
        ...
        public IActionResult Index(int? page, string search, int pageSize = 5)
        {
            if (HttpContext.Session.GetString("TenDangNhap") != null && (HttpContext.Session.GetString("VaiTro") == "Admin"))
            {
                ...
                IQueryable<ThanhThoaiRestaurant.Models.OCung> foodItems = _context.OCungs;


                if (!string.IsNullOrEmpty(search))
                {
                    foodItems = foodItems.Where(item =>
                    item.DungLuong.Contains(search) ||
                    item.MaOC.ToString().Contains(search));
        ...
        public IActionResult Create(OCung menuItem)
        {
            // Kiểm tra xem MaNhom đã tồn tại trong CSDL hay không
            if (_context.OCungs.Any(m => m.MaOC == menuItem.MaOC))
            {
                ModelState.AddModelError("MaOC", "Mã ổ cứng đã tồn tại trong CSDL.");
                return View(menuItem); // Trả về view với thông báo lỗi
            }

            _context.OCungs.Add(menuItem);
            _context.SaveChanges();

            return RedirectToAction("Index", "AdminOCung");
        }
        ...
        {
            var menuItem = _context.OCungs

                .FirstOrDefault(m => m.MaOC == id);
        ...
        {
            var menuItem = _context.OCungs.FirstOrDefault(m => m.MaOC == id);
        ...
        [HttpPost]
        public IActionResult Edit(int id, OCung menuItem)
        {
            // Kiểm tra xem món ăn có tồn tại trong cơ sở dữ liệu hay không
            var existingMenuItem = _context.OCungs.Find(id);
            ...
            existingMenuItem.DungLuong = menuItem.DungLuong;
        ...
        public IActionResult Delete(int id)
        {
            ...
            var relatedMonAn = _context.MonAns.Any(ma => ma.MaOC == id);
            if (relatedMonAn)
            {
                // Thêm thông báo lỗi và trả về view với thông báo lỗi
                TempData["ErrorMessage"] = "Không thể xóa vì có sản phẩm dung lượng này.";
                return RedirectToAction("Index");
            }

            // Xóa NhomMonAn
            _context.OCungs.Remove(menuItem);
            _context.SaveChanges();
        ...
        {
            var hasProducts = _context.OCungs.Any(p => p.MaOC == id);
