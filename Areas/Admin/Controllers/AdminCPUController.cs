...
namespace ThanhThoaiRestaurant.Areas.Admin.Controllers
{
    ...
    public class AdminCPUController : Controller
    {
        private readonly QuanLyNhaHangContext _context;
        private readonly IWebHostEnvironment _webHostEnvironment;
        public AdminCPUController(QuanLyNhaHangContext context, IWebHostEnvironment webHostEnvironment)
        {
            _context = context;
        ...
        public IActionResult Index(int? page, string search, int pageSize = 5)
        {
            if (HttpContext.Session.GetString("TenDangNhap") != null && (HttpContext.Session.GetString("VaiTro") == "Admin"))
            {
                ...
                IQueryable<ThanhThoaiRestaurant.Models.CPU> foodItems = _context.CPUs;


                if (!string.IsNullOrEmpty(search))
                {
                    foodItems = foodItems.Where(item =>
                    item.TenLoaiCPU.Contains(search) ||
                    item.MaCPU.ToString().Contains(search));
        ...
        public IActionResult Create(CPU menuItem)
        {
            // Kiểm tra xem MaCPU đã tồn tại trong CSDL hay không
            if (_context.CPUs.Any(m => m.MaCPU == menuItem.MaCPU))
            {
                ModelState.AddModelError("MaCPU", "Mã CPU đã tồn tại trong CSDL.");
                return View(menuItem); // Trả về view với thông báo lỗi
            }

            _context.CPUs.Add(menuItem);
            _context.SaveChanges();

            return RedirectToAction("Index", "AdminCPU");
        }
        ...
        {
            var menuItem = _context.CPUs

                .FirstOrDefault(m => m.MaCPU == id);
        ...
        {
            var menuItem = _context.CPUs.FirstOrDefault(m => m.MaCPU == id);
        ...
        [HttpPost]
        public IActionResult Edit(int id, CPU menuItem)
        {
            // Kiểm tra xem món ăn có tồn tại trong cơ sở dữ liệu hay không
            var existingMenuItem = _context.CPUs.Find(id);
            ...
            existingMenuItem.TenLoaiCPU = menuItem.TenLoaiCPU;
        ...
        public IActionResult Delete(int id)
        {
            ...
            // Kiểm tra xem có MonAn chứa MaCPU này không
            var relatedMonAn = _context.MonAns.Any(ma => ma.MaCPU == id);
            if (relatedMonAn)
            {
                // Thêm thông báo lỗi và trả về view với thông báo lỗi
                TempData["ErrorMessage"] = "Không thể xóa vì có sản phẩm thuộc CPU này.";
                return RedirectToAction("Index");
            }

            // Xóa CPU
            _context.CPUs.Remove(menuItem);
            _context.SaveChanges();
        ...
        {
            var hasProducts = _context.MonAns.Any(p => p.MaCPU == id);
